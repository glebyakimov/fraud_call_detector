...
Зачем это нужно:
- TF‑IDF хорошо ловит общую лексику, но “смысловые маркеры” мошенничества полезно выделять отдельно.
- Триггеры (банк/смс/перевод/полиция/AnyDesk/и т.д.) дают стабильный сигнал даже при ошибках ASR.
- Мета-признаки помогают модели учитывать “стиль” (много цифр, много !?, много пунктуации и т.п.).

Как устроены триггер-группы:
- `groups` — это список групп, где каждая группа = список слов/фраз.
- Для каждой группы считаем количество совпадений в тексте и превращаем в “мягкий” скоринг 0..1.
- Матчинг сделан аккуратно:
  - одно слово → матч по токенам
  - фраза с пробелом → матч подстрокой
  - слово с дефисом → либо матч подстрокой, либо “все части встречаются как токены”
"""
...
def _keyword_hits(t_match: str, kw_raw: str) -> int:
    # Возвращает 1, если ключ “сработал” на тексте.
...
def numeric_meta(text: str) -> np.ndarray:
    # [log_len, digit_ratio, upper_ratio, excl, quest, log_tokens, punct_ratio]
    raw = text or ""
